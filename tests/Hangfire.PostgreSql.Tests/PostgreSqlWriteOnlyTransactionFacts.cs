    ...
    public class PostgreSqlWriteOnlyTransactionFacts : IClassFixture<PostgreSqlStorageFixture>
    {
      ...
      ";
            ...
            this.UseConnection(connection =>
            {
                ...
                this.Commit(connection, x => x.AddJobState(jobId, state.Object));
        ...
        public void AddToQueue_CallsEnqueue_OnTargetPersistentQueue()
        {
            this.UseConnection(connection =>
            {
                ...
                this._fixture.PersistentJobQueueProviderCollection.Add(correctProvider.Object, new[] { "default" });

                try
                {
                    this.Commit(connection, x => x.AddToQueue("default", "1"));

                    correctJobQueue.Verify(x => x.Enqueue(connection, "default", "1"));
                }
                finally
                {
                    this._fixture.PersistentJobQueueProviderCollection.Remove("default");
                }
            });
        }
        ...
        public void IncrementCounter_AddsRecordToCounterTable_WithPositiveValue()
        {
            this.UseConnection(connection =>
            {
                this.Commit(connection, x => x.IncrementCounter("my-key"));
        ...
        public void IncrementCounter_WithExpiry_AddsARecord_WithExpirationTimeSet()
        {
            this.UseConnection(connection =>
            {
                this.Commit(connection, x => x.IncrementCounter("my-key", TimeSpan.FromDays(1)));
